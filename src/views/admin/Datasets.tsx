...
export default function Datasets({ snap, onNavigate }: { snap: SimSnapshot; onNavigate?: (tab: string) => void }) {
  ...
  return (
    <div className="space-y-3">
      <Panel
        ...
        right={
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-gray-500">카드 클릭 시 피처·샘플</span>
            ...
            return (
              ...
              >
                ...
                <div className="mt-2.5 flex items-end gap-3">
                  ...
                  <Stat label="피처" value={`${d.featureCount}`} />
      ...
      <Panel title="스키마 고정 — 위험운전 8종이 그대로 피처 축이 된다" right={<span className="text-[11px] text-gray-500">공단 표준 DTG 409</span>}>
      ...
      <Drawer
        ...
        title={open?.name ?? ''}
        sub={open ? `${fmt(open.rows(snap))}행 · 피처 ${open.featureCount}개 · 품질 ${open.ready}점 · 결측 ${open.missing}%` : ''}
      >
    ...
    <>
      ...
      <Sec t="라벨 — 무엇이 정답인가">
        ...
        <div className="mt-1.5 break-keep text-[11.5px] leading-relaxed text-gray-500">
          라벨이 없는 데이터는 학습셋이 아니라 그냥 로그입니다. Qdrive는 <b className="text-gray-300">담당자의 승인·판정이 그대로 라벨이 되도록</b>{' '}
          업무 흐름을 설계했습니다.
        </div>
      </Sec>

      <Sec t="피처 정의" right={<span className="text-[11px] text-gray-500">전체 {ds.featureCount}개 중 대표 {ds.features.length}개</span>}>
          ...
          <table className="w-full text-left text-[11.5px]">
            <thead>
              <tr className="border-b border-gray-800 text-[10.5px] text-gray-500">
                <th className="py-1.5 pr-3 font-semibold">피처</th>
