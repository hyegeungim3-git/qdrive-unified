...
## 💬 온톨로지 질의 탭 + 운행유형·차고지 축 (2026-08-19)
...
**화면** `views/OntologyChat.tsx` = 최상위 탭 '💬 온톨로지 질의'(admin 다음). 규칙 경로가 기본이고, 자유 질문만 라이브 Claude(`claude-opus-5`, 사용자 키 localStorage)로 넘기되 그 답에는 **'계산이 아니라 설명'** 꼬리표를 붙인다. 못 알아들으면 지어내지 않고 무엇이 있으면 답할 수 있는지 말한다.
