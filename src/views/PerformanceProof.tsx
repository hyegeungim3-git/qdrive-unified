 ...
 * 전략 문서 §6(유의미한 결과의 4단 증명: 기준선 → 효과 확인 → 검증 → 정직한 불확실성)의 실동작판.
 * 핵심: 엔진이 이미 '코칭을 안 했다면 썼을 연료(baselineFuelM3)'를 함께 계산하므로,
 * 실측(fuelM3)과의 차이가 곧 서비스가 만든 순수 절감이다 — 유가·날씨 같은 외부요인이 제거된 값.
 */
...
type Svc = { icon: string; name: string; result: string; source: string; sample: string; cross: string; status: '검증' | '제출' | '교차' }
...
export default function PerformanceProof() {
  ...
  // ── ① 효과 확인 — 코칭 없을 때 예상(baseline) vs 실측 ──
  ...
  const attrData = [
    { name: '코칭 없이 예상', v: baseFuel, kind: 'base' },
  ...
  const STATUS_CLS = {
    ...
    제출: 'bg-violet-500/15 text-violet-300',
  } as const
  ...
  return (
    <div className="mx-auto flex h-full max-w-6xl flex-col gap-3 overflow-y-auto pr-1">
      ...
      {/* ── ① 효과 확인 (코칭 없을 때와 비교) ── */}
      <Panel
        title="① 효과 확인 — 코칭 없을 때와 비교"
        right={<span className="text-[11px] text-gray-500">유가·날씨 영향 제거된 순수 절감</span>}
      >
        <div className="grid grid-cols-[1.1fr_1fr] gap-4 max-[820px]:grid-cols-1">
          ...
          <div className="flex flex-col justify-center gap-3">
            <div className="rounded-xl border border-amber-500/25 bg-amber-500/5 px-5 py-4">
              <div className="text-[12px] font-semibold text-amber-300">서비스가 만든 순수 절감 (연료)</div>
              ...
              <div className="mt-1 text-[12px] leading-relaxed text-gray-400">
                엔진이 <b className="text-gray-200">코칭을 안 했다면 썼을 연료</b>를 함께 계산하므로,
                실측과의 차이는 유가·날씨가 아니라 <b className="text-amber-300">서비스(코칭)</b>가 만든 효과예요.
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg bg-gray-800/40 px-3 py-2 text-center">
                <div className="text-[10px] text-gray-500">코칭 없이 예상</div>
      ...
      <Panel title="④ 서비스별 성과 · 신뢰지표 (투명성)" right={<span className="text-[11px] text-gray-500">성과마다 '얼마나 믿을 수 있는지'를 함께</span>}>
        ...
        <div className="mt-2 text-[10px] leading-relaxed text-gray-600">
          단일 소스는 오차·조작 가능 — 모든 성과는 <b className="text-gray-400">두 소스 교차검증</b> 또는 검증기관 제출로 재확인됩니다.
