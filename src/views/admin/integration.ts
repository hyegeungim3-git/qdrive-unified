...
export const INTEGRATION: Record<string, Integration> = {
  'DTG 409': {
    ...
    retry: '지수 백오프 최대 5회 · 미전송분은 단말 로컬 큐 보관(최대 72h)',
    ...
    transforms: [
      ...
      'simTime을 운행 단위 구간에 매핑 — 구간 밖이면 보류 큐로',
  ...
  'DTG 521': {
    ...
    transforms: [
      ...
      'eTAS 제출 원본과 동일 레코드 유지 — 법정 기록은 변형하지 않음',
    ],
    mapping: [
  ...
  'BIS 공개 API': {
    ...
    transforms: [
      ...
      '3초 스트림을 1초 축으로 보간하지 않음 — 원 해상도를 유지해 교차검증용으로만 사용',
      '정류소명을 정류장 인스턴스에 매칭 — 미매칭은 보류 큐로',
    ],
    mapping: [
  ...
  '단말 상태': {
    ...
    joinKeys: ['vehicleId → 차량', '결손 구간 → 품질 룰 Q1 원인 태그'],
    transforms: [
      '하트비트 누락 구간을 데이터 결손 구간으로 표시',
      '결손 원인(통신·저장·전원)을 Q1 격리 레코드에 자동으로 붙임',
    ],
    mapping: [
...
export const PENDING_PREP: Record<string, { need: string; ready: string; blocked: string }> = {
  AFC: { need: 'iM유페이 데이터 제공 협약', ready: '스키마 정의·가명처리 설계 완료', blocked: '협약 체결 대기' },
  ...
  'BMS 배차원장': { need: '대구시 BMS 조회 권한', ready: '계획-실적 대조 스키마 설계 완료', blocked: '시 협조 대기' },
...
export function connLogs(s: SimSnapshot, code: string): LogLine[] {
  ...
  if (code === 'OBD/CAN' && s.fault) {
    out.push({ at: clock(s.fault.startedAt), level: '경고', msg: `${shortId(s.fault.vehicleId)} coolantTemp 정상범위 이탈 — 품질 룰 Q2 보류, 고장 예측으로 회부` })
  }
  if (code === '정비이력') {
    out.push(
      s.workOrders.length > 0
        ? { at: clock(s.workOrders[0].createdAt), level: '정보', msg: `작업지시 ${s.workOrders.length}건 수신 — 고장 예측 라벨로 부착 완료` }
        : { at: clock(t), level: '정보', msg: '오늘 신규 작업지시 없음 — 이벤트 기반 원천이라 정상' },
    )
  }
  if (code === 'BIS 공개 API') {
    out.push({ at: clock(Math.max(0, t - 240)), level: '경고', msg: '공개 API 응답 3.4초 — 폴링 주기(3초) 초과 1회, 자동 백오프 후 정상' })
  }
  if (code === '민원' && s.complaints.length > 0) {
    out.push({ at: clock(s.complaints[0].simTime), level: '정보', msg: `민원 ${s.complaints.length}건 접수 — 증빙 자동매칭 큐로 전달` })
  }
  if (code === 'RTK') {
    out.push({ at: clock(Math.max(0, t - 520)), level: '정보', msg: 'NTRIP 보정신호 재연결 — Float 12초 후 Fixed 복귀' })
  }
  out.push({ at: clock(Math.max(0, t - 60)), level: '정보', msg: '스키마 검증 통과 · 적재 정상' })
  out.push({ at: clock(0), level: '정보', msg: '커넥터 기동 — 인증·엔드포인트 확인 완료' })
  return out
}
