...
const PHASES: {
...
}[] = [
  {
    ...
    items: [
      ...
      { source: 'BIS 실차 위치', desc: '공개 API 조회 연계 — 이 데모 지도에 실차 위치를 겹쳐 표시 중 · 대구 초정밀 버스(3초 주기)와 동일 계열' },
      ...
      <div className="rounded-xl border border-sky-500/20 bg-sky-500/5 px-5 py-4">
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {[
            ...
            ['탄소 크레딧 발행 지원', '5개사 묶음 발행 절차 지원', '2차'],
            ['기사 안전 이력 증명', '이동 가능한 경력 포트폴리오', '2차'],
            ['ESG·의회 보고 자동화', '공시·의회 대응 리포트 정기 제공', '2차'],
        ...
        <div className="mt-2 text-[11px] leading-relaxed text-gray-500">
          전부 같은 운행 단위(Trip) 중심축 위에서 열립니다 — 각 항목은 성과가 검증된 뒤에 차수를 나눠 여는
          것이며, 1차 실증 범위에는 포함되지 않습니다.
        </div>
      </div>
